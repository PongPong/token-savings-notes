...
| laya-mlx (mizorewww) | https://github.com/mizorewww/laya-mlx · [BENCHMARKS.md](https://github.com/mizorewww/laya-mlx/blob/main/BENCHMARKS.md) · [README.zh-CN.md](https://github.com/mizorewww/laya-mlx/blob/main/README.zh-CN.md) | Independent **native MLX** runtime for Laya on Apple Silicon (~3.8k★ as of ~2026-09-22, **Apache-2.0**, Python). Same `choice`/`score`/`noul`. No text generation, no PyTorch/Transformers runtime, no cloud inference API. PyPI `laya-mlx`. HF `aac6fef/laya-mlx`, `aac6fef/laya-multilingual-mlx`, `aac6fef/laya-typed-decisions-mlx`. **Not** official Convai; RLCD/fine-tune stay upstream. Author M3 Max FP16: EN 1-q P50 **13.42 ms** / P95 **13.92 ms**; multilingual P50 **7.39 ms** / P95 **7.79 ms**; 50-q **146.8 q/s** EN / **395.0 q/s** multilingual (`batch_size=64`); peak **943.6** / **687.6 MiB**; **0 output tokens**. Port fidelity **63/63** FP32 and FP16 (**378/378**) on those fixtures, not universal accuracy. Snake optimize (author): **75.40 moves/s** — gameplay latency, not a token-%. | **Author latency (ms) / 0 output tokens** — **not** a session token-% |
| HF blog — Jev vs djev vs Laya vs OpenJev vs SemIf | https://huggingface.co/blog/sora-2/jev-ai-vs-djev-vs-laya-vs-openjev-vs-semif-which-d | “Which Decision Model Should You Use?” Published **23 Sep 2026**. HF account alchemication-pfpt. Article author (sora-2) replied in comments with links: [Jev docs](https://docs.typesafe.ai/introduction), [djev](https://github.com/Davipar/djev-dev), [Laya](https://github.com/NandhaKishorM/laya), [OpenJev](https://github.com/razorback16/openjev), [SemIf](https://github.com/TheoLeeCJ/SemIf-OpenJev). Five structured-decision stacks. JevBench v1.3.0 (Sep 2026): 52 systems, 534 decisions. Snapshot table lives in [TYPESAFE-JEV](./TYPESAFE-JEV.md#alternatives-landscape-decision-backends). djev preview pricing: **verify live**. | **not a token-%** (JevBench composite / cal / latency) |
...
- [Jev ai vs djev vs Laya vs OpenJev vs SemIf](https://huggingface.co/blog/sora-2/jev-ai-vs-djev-vs-laya-vs-openjev-vs-semif-which-d) — 23 Sep 2026 — HF blog (alchemication-pfpt; author comment sora-2). Decision-backend comparison. Scores in [TYPESAFE-JEV](./TYPESAFE-JEV.md#alternatives-landscape-decision-backends). **Not a token-%.**
- [djev](https://github.com/Davipar/djev-dev) — GitHub (hosted speed + image/camera decision API; preview pricing — verify live)
- [OpenJev](https://github.com/razorback16/openjev) — GitHub (self-hosted Jev-compatible `/v1/systemone`)
- [SemIf](https://github.com/TheoLeeCJ/SemIf-OpenJev) — GitHub (open-model logit reader; MIT code)
...
---
## Web primary (25 Sep 2026)

HF blog, published 23 Sep 2026. Account alchemication-pfpt. Article author sora-2 posted repo links in the comments.

- Comparison of five structured-decision stacks. JevBench v1.3.0 composite, calibration, and speed live in [TYPESAFE-JEV Alternatives landscape](./TYPESAFE-JEV.md#alternatives-landscape-decision-backends).
- Savings column: **not a token-%** (JevBench composite / cal / latency). Do not copy those scores onto the NOTES cheat sheet.
- [djev](https://github.com/Davipar/djev-dev) preview pricing: **verify live**. [OpenJev](https://github.com/razorback16/openjev). [SemIf](https://github.com/TheoLeeCJ/SemIf-OpenJev). Laya repo already in the named table.

---
**Maintenance (weekly, Monday ~09:00 Europe/London):** Refresh X/web deltas. Re-check metering tools and **[TYPESAFE-JEV.md](./TYPESAFE-JEV.md)** (including the decision-backend landscape). Update savings figures only when attributed. JevBench composite / cal / latency stays **not a token-%**. Keep `SPEAKER-NOTES.md` in sync if the talk changes. Last edit: 2026-09-25. Source of truth: private GitHub `PongPong/token-savings-notes`.
